...
1. **Product direction:** which deferred item (if any) above to scope next, with a real answer to the policy question each one raises.
2. **Production launch track:** work through `execution/PRODUCTION_READINESS.md`'s migration checklist and the 4 open blockers in `execution/BLOCKERS.md` — provisioning Supabase, choosing a rate-limit backend, choosing evidence storage, and getting a real legal/privacy review, none of which this session can do unilaterally.
