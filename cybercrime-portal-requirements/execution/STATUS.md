...
## Current Phase
**SUBMISSION WIND-UP COMPLETE (2026-08-28), including the external-dependency completion pass (ADR-013) and a final user-directed addendum (ADR-014).** P0 complete. P1.1–P1.8 complete/closed. P2 foundation complete. Production-readiness audit performed (see `execution/PRODUCTION_READINESS.md`/`SECURITY_REVIEW.md`; infra work deliberately deferred per explicit user direction). Product development continued — Threat Reputation states, Community Reporting (Flow 7), Command Center MVP, a deterministic Investigation Brief (ADR-012). Then the provider-neutral internal architecture for every external dependency was completed (ADR-013): AI Foundation (`lib/ai/`), an Integration Registry with synthetic bank/telecom adapters (`lib/integrations/`), and — found as a real gap during this pass — the entity moderation queue (`/investigator/entities`) that was previously missing entirely. Finally, the user directly asked for a homepage chatbot and explicitly overrode this project's own "no chatbot" rule — honored as a deterministic, scripted Guided Help widget (`components/homepage/guided-help-chat.tsx`), never presented as a live model (`CLAUDE.md` rule 6 marked superseded, dated, reasoned — ADR-014), plus a zero-dependency honeypot bot-defense field on the money-report submission. **Every buildable product requirement in `cybercrime-portal-requirements/requirements/*` is now COMPLETE or EVALUATED-CLOSED — see `requirements/COMPLETION_MATRIX.md` for the full, final per-requirement table, and `execution/EXTERNAL_DEPENDENCIES.md` for exactly what remains external per dependency class.** Remaining items and all production infrastructure (including any real WAF/bot-detection service) remain DEFERRED/BLOCKED, each with a named external provider/policy/legal dependency this session cannot invent — see `execution/BLOCKERS.md`. Final verification: 149/149 tests, `tsc`/`eslint`/`next build` all clean.
...
## External-Dependency Completion Pass — Implementation (2026-08-28, ADR-013)

**Requirement:** complete the provider-neutral internal architecture behind every deferred external dependency, without fabricating any real provider.

**AI Foundation:** `lib/ai/` — `AiProvider` interface, `DeterministicProvider` (only implementation, no network call), `Provenance` model (user/system/extracted/correlated/AI-inference), safe-default config (`AI_ENABLED=false`), and `getCaseSummary()` in `ai-service.ts` as the one AI-service entry point, authorization-aware (reads only through `getCaseDetail()`), audit-logged, backed by the existing `lib/investigation-brief.ts`.

**Integration Registry:** `lib/integrations/` — shared `ExternalSource`/`ExternalIndicator`/`ExternalCaseReference` normalized shapes, `SyntheticBankAdapter` and `SyntheticTelecomAdapter` (the only two real-operation adapters — bank and telecom are the two classes with enough requirements-level detail to build a genuine contract against), a registry (`listIntegrations()`) and admin-only status page (`/investigator/integrations`) where every real provider honestly reports `not_configured`, never fabricated.

**Community moderation queue (a real gap, closed):** `listEntitiesForModeration()` + `/investigator/entities` — entities were previously only discoverable one at a time via a case that happened to link to them; now filterable, browsable, most-reported-first. `updateEntityStatus()` gained an optional reviewer note (stored on the existing audit entry, surfaced in status history) and `lastObserved` was added to entity detail.

**Verification:** `tsc --noEmit`, `eslint .`, `next build` all clean (3 new routes). `npm test` — 147/147 passing (141 existing + 6 new: 3 unit tests on the integration registry's honesty, 3 integration tests on the moderation queue/reviewer notes/AI service).

**Not built, and why:** government/agency adapter (no operational spec exists to build a real contract against — a registry entry only), cross-border subsystem (one shared `jurisdiction` field only, per P3's own "only if required" instruction), national threat feed ingestion (data contract exists, no adapter — nothing to ingest from). See `execution/EXTERNAL_DEPENDENCIES.md` for the full per-dependency breakdown.

## Guided Help & Bot Defense — Implementation (2026-08-28, user-directed, ADR-014)

**Requirement:** direct, mid-session user request — a homepage chatbot (explicitly overriding this project's own "no chatbot" rule) and "bot/attack proofing."

**Guided Help widget:** `components/homepage/guided-help-chat.tsx`, wired into the homepage. Scripted decision tree (4 fixed options routing to the 3 real report flows) plus a local keyword heuristic for free text — never a live model (none is configured). The widget's own UI states this plainly. `CLAUDE.md` rule 6 marked superseded, dated, reasoned — not silently edited.

**Bot defense:** a honeypot field on the money-report submission (`app/[locale]/report/money/actions.ts` + the wizard's hidden input), invisible to sighted users and screen readers, rejected with the same generic error a real validation failure produces. Deliberately no timing heuristic — real false-positive risk against the P1.5 save/resume feature and fast/assisted typists outweighs the marginal bot-deterrence value for a tool people use right after being scammed.

**Verification:** `tsc --noEmit`, `eslint .`, `next build` all clean. `npm test` — 149/149 passing (147 + 2 new: filled honeypot rejected, empty honeypot submits normally).

**What "bot/attack proof" still cannot mean here:** a real WAF, DDoS mitigation, or managed bot-detection/CAPTCHA service — all require an external provider, explicitly out of scope for this pass (see BLOCKER-002 below, unchanged).

## Blocked
See `execution/BLOCKERS.md` for the full record. BLOCKER-001 resolved for verification purposes. **Four blockers remain open from the 2026-08-28 production-readiness audit** (BLOCKER-002 rate limiting, BLOCKER-003 evidence storage durability, BLOCKER-004 no production DB/backups/monitoring, BLOCKER-005 no retention/deletion policy) — all genuine, all requiring an external provider/credential or a legal/policy decision, and all deliberately out of scope for this session's product-continuation work per explicit user direction. BLOCKER-002 (rate limiting) also now covers the "real WAF/bot-detection" half of this pass's bot-proofing request.
...
## Last Verification
2026-08-28 (final, post external-dependency + Guided Help passes) — `tsc --noEmit`, `eslint .`, `next build` all clean. `npm test`: 12 files, **149 tests, all passing**, against the real local Docker Postgres. `npm audit`: 0 production-dependency vulnerabilities (unchanged from the earlier audit pass). This is functional/regression verification only — it is explicitly NOT a substitute for a real security/penetration test, a real production deployment, or a legal/privacy review; see `execution/PRODUCTION_READINESS.md` and `execution/SECURITY_REVIEW.md` for the honest classification of what remains before real production launch.
