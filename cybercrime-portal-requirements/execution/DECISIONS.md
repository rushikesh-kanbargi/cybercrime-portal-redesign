...
### ADR-013 — Provider-neutral architecture for AI and external integrations: interfaces + one synthetic implementation each, nothing more
**Date:** 2026-08-28
**Status:** Accepted

**Context**
Following the P2 product-continuation batch, the user explicitly asked for the internal, provider-independent architecture behind every deferred external dependency (AI, bank, telecom, government, community moderation, national data) to be completed — while repeating, just as explicitly, the rule against fabricating any real provider, credential, or government connection. The risk on the other side is real too: building five near-identical adapter stubs with zero callers is exactly the "no interface with one implementation" anti-pattern this project's own global instructions warn against. The resolution is to build genuinely reusable, minimally-scoped contracts with exactly one real (deterministic/synthetic) implementation each — concrete enough to be tested and to demonstrate the seam, not speculative enterprise scaffolding.

**Decision**
1. **AI Foundation** (`lib/ai/`) — `AiProvider` interface, `DeterministicProvider` (the only implementation; never makes a network call), `AiConfig` (safe defaults, no fake key), and `getCaseSummary()` in `ai-service.ts` as the one AI-service entry point with authorization-aware retrieval (reads only through the existing `getCaseDetail()`) and a `Provenance` model distinguishing user/system/extracted/correlated/AI-inference facts. Backed by the already-shipped `lib/investigation-brief.ts`, not a second implementation of it.
2. **Integration Registry** (`lib/integrations/`) — one shared `IntegrationDescriptor`/`ExternalSource` contract, a `SyntheticBankAdapter` and `SyntheticTelecomAdapter` (the only two adapters implemented — bank and telecom are the two classes `12-financial-intelligence.md`/`18-integrations.md` give enough concrete operation detail to build a real contract against), and a registry (`listIntegrations()`) that every "real" provider entry reports as `not_configured`/`provider: "none"`, never fabricated. A new admin-only page (`/investigator/integrations`) shows this honestly — "Not configured," never a misleading "No data found," per the user's own explicit instruction.
3. **National/government/cross-border**: contract-only, no adapter built. Unlike bank/telecom, no requirements file gives enough operational detail (what does "search," "lookup," or "referral" actually mean for a government agency this app has no relationship with?) to build a real contract against — building one anyway would be inventing the shape of a government workflow, which `AGENTS.md`'s anti-hallucination rule and `18-integrations.md`'s own "never fake production integration" instruction both argue against. Cross-border gets exactly one shared optional field (`jurisdiction` on `ExternalSource`), not a subsystem — P3's own instruction to add this "only if required."
4. **Community Moderation — the real gap, closed.** Auditing the existing Community Reporting feature (P2, previous pass) found it had no *queue*: a citizen-reported entity was only discoverable by an investigator who happened to already be looking at a case linking to it. Built `listEntitiesForModeration()` + `/investigator/entities` (a real, previously-missing browsable list, filterable by the 9-state status enum) and an optional reviewer note on `updateEntityStatus()`, stored on the existing audit entry (no new table). This is the "Review" step of the Submission→Validation→Queue→Review→Decision→Audit workflow the instructions named — Submission/Validation/Decision/Audit already existed from the prior ADR-012 pass.
5. **Explicitly declined mid-session:** a citizen-facing conversational AI chatbot, requested directly by the user. `CLAUDE.md`'s own hard rule 6 (this project's, not invented by this session) forbids it regardless of AI-feature authorization — stated directly to the user rather than silently built or silently ignored.

**Alternatives Considered**
- Building all five adapter classes (bank, telecom, government, platform, agency) uniformly. Rejected: government/agency/platform have no operational specification anywhere in the requirements to build a real contract from — doing so would mean inventing one, the opposite of what "provider-neutral" is supposed to mean.
- Wiring `getCaseSummary()` into the case-detail UI as a second, parallel rendering path alongside the existing Investigation Brief. Rejected: would duplicate a feature that already ships and is already tested, for no user-visible gain — the AI-service layer's value this pass is the formal, testable contract, not a UI change.
- A full `crossBorderCase`/`jurisdiction` table set. Rejected — no real requirement detail exists to design it correctly, and an empty table with no writer is dead schema.

**Rationale**
"Build the internal capability, defer only the genuinely external portion" is satisfied precisely where a real requirements-level contract exists (AI summarization, bank/telecom lookups, community moderation review) and deliberately not extended to where it doesn't (government referral protocols, cross-border legal workflows) — the line is drawn by what's actually specified, not by what would look impressive.

**Consequences**
- Positive: adding a real AI provider or a real bank/telecom API later requires implementing one interface each and registering it — no changes to any existing call site.
- Positive: closed a real, previously-undiscovered gap (no entity moderation queue existed) as a direct result of doing this pass properly rather than treating it as pure architecture theater.
- Negative: the synthetic bank/telecom adapters have no live caller in the running application (by design — they exist to make the contract concrete and testable, not to be used yet). Documented, not hidden.
- Operational: `AI_ENABLED`, `BANK_INTEGRATION_ENABLED`, `TELECOM_INTEGRATION_ENABLED`, `GOVERNMENT_INTEGRATION_ENABLED` are real env vars with safe (off/none) defaults and, honestly, no implementation behind them yet if flipped on — the registry page states this rather than letting a flag imply more than exists.

**Verification / Evidence**
`lib/ai/{types.ts,config.ts,deterministic-provider.ts,ai-service.ts}` (new), `lib/integrations/{types.ts,registry.ts,synthetic-bank-adapter.ts,synthetic-telecom-adapter.ts}` (new), `app/investigator/integrations/page.tsx` (new, admin-only), `app/investigator/entities/page.tsx` (new — the moderation queue), `lib/actions/entity-intelligence.ts` (`listEntitiesForModeration`, reviewer-note support on `updateEntityStatus`, `lastObserved`), `components/investigator/entity-status-control.tsx` (note input). Tests: `tests/unit/integrations-registry.test.ts` (new, 3 tests — no real integration ever reports healthy/configured, synthetic adapters are labelled, deterministic AI is enabled while the real one isn't), `tests/integration/case-management.test.ts`'s new "External-dependency pass" block (3 tests — moderation queue excludes synthetic data and filters correctly, a reviewer note is stored and surfaced without requiring a status change, `getCaseSummary` is grounded/labelled/authorization-enforced). Full suite: **147/147 passing**. `tsc --noEmit`, `eslint .`, `next build` all clean (3 new routes confirmed: `/investigator/entities`, `/investigator/entities/[id]` unchanged path now list-backed, `/investigator/integrations`).

---

### ADR-014 — Guided Help widget: user-directed override of the "no chatbot" rule, kept honest via a hard non-negotiable
**Date:** 2026-08-28
**Status:** Accepted

**Context**
Mid-session, the user asked directly for a homepage chatbot and explicitly instructed that this project's own "no chatbot" rule (`CLAUDE.md` #6, a hard rule the user themselves authored earlier) be overridden. Separately, the user asked for the site to be made "bot/attack proof." Both requests needed the same honesty discipline this entire session has applied to AI: the user can change their own project's product rules, but no instruction — from the user or otherwise — can make a fake integration real, or make a "chatbot" a real conversational AI when no provider is configured.

**Decision**
1. **Guided Help widget** (`components/homepage/guided-help-chat.tsx`) — a fixed-position, dismissible chat-style UI on the homepage. Entirely scripted: four fixed quick-reply options routing to the three real report flows (plus an "I'm not sure" default), and a free-text input matched against a small local keyword heuristic (same "rules floor" precedent as `lib/classify.ts` — not a language model). The widget's own UI states plainly, in the interface itself, that this is "scripted guidance, not a live AI conversation." `CLAUDE.md` rule 6 is marked superseded, dated, and reasoned, the same pattern ADR-001 already established for rule 5 — not silently edited away.
2. **Bot defense** — a honeypot field on the money-report submission (`app/[locale]/report/money/actions.ts`), invisible to sighted users and screen readers alike (off-screen positioning + `aria-hidden` + `tabIndex={-1}`, never `display:none`, which some bots specifically detect and skip). A filled value produces the exact same generic error a real validation failure would, so a bot gets no signal it was caught differently. Deliberately **not** paired with a timing-based heuristic (reject submissions "too fast") — that would risk a false positive against this app's own P1.5 save/resume feature (a citizen resuming an already-filled draft could submit within seconds) or against a fast/assisted typist, and this is a tool people use immediately after being scammed — blocking a real victim's report is a worse failure mode than letting one more bot through the existing rate limiter.
3. **What "bot/attack proof" still cannot honestly mean here:** a real WAF, DDoS mitigation, or managed bot-detection service (Cloudflare, hCaptcha, etc.) requires an actual external provider — explicitly out of scope for this pass per the user's own instruction in the same message, and unchanged from `execution/BLOCKERS.md`'s BLOCKER-002.

**Alternatives Considered**
- A free-text-only chatbot routing directly through `lib/classify.ts`'s money-flow subcategory classifier for all three report types. Rejected: that classifier was built and tuned only for financial fraud; using it to route a harassment or account-compromise disclosure risks a wrong, possibly re-traumatizing misclassification. The quick-reply-first design avoids this; free text only ever suggests, never silently redirects.
- A CAPTCHA. Rejected: no CAPTCHA provider is configured (same anti-fabrication constraint as everything else this session), and a real one is exactly the kind of external service explicitly deferred.

**Rationale**
The user's authority over their own project's product rules is real and honored directly (no silent refusal, no silent compliance without updating the durable record). The constraint against fabricating a live AI or a real security service is not a "caution setting" that authority can waive — it's the same honesty rule that has governed every AI/integration decision this entire session, and it protects the user's own submission from making a false claim, not just this session's own judgment.

**Consequences**
- Positive: the site now has an interactive, on-brand triage aid that was explicitly requested, without pretending it's something it isn't.
- Positive: a real, zero-false-positive-risk bot deterrent added to the highest-traffic public write path.
- Negative: the Guided Help widget's free-text matching is a simple keyword heuristic, not true intent understanding — a citizen describing something unusual may get a generic "I'm not sure" routing. Acceptable: the four options are always visible, and every route leads to a real, working report flow either way.
- Negative: honeypot-only bot defense is a modest deterrent, not a real mitigation against a sophisticated attacker — disclosed here and in `execution/BLOCKERS.md`, not oversold.

**Verification / Evidence**
`components/homepage/guided-help-chat.tsx` (new), `app/[locale]/page.tsx` (wired in), `CLAUDE.md` (rule 6 marked superseded, dated, reasoned), `app/[locale]/report/money/actions.ts` (`honeypot` field + check), `app/[locale]/report/money/money-report-wizard.tsx` (hidden honeypot input, never persisted to the localStorage draft). Tests: 2 new (`tests/integration/reporting-and-entity-extraction.test.ts` — a filled honeypot is rejected; an empty one submits normally, unaffected). Full suite: **149/149 passing**. `tsc --noEmit`, `eslint .`, `next build` all clean.
