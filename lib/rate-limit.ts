...
export function getClientIp(headers: Headers): string {
  ...
  // Takes `Headers` directly (not a `Request`) so server actions — which
  // only have `next/headers`' `headers()`, no `Request` object — can call
  // this the same way the API routes do.
