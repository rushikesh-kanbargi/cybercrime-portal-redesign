...
**Phase I (design-tool polish pass on the D54 nav dropdowns and five new content pages) complete — 2026‑08‑26. See D55.**

- **Why:** Phase H's five new content pages and four nav dropdowns were built for structural/content completeness, not visual craft, and were explicitly the least-polished surface in the app relative to the already-shipped `/faq`/`/safety-tips` bar. This phase ran the mandated design-tool stack against that surface only and fixed what was actually found under-polished — no rebuild, no new visual language.
- **Two real, scoped defects found and fixed**, both by reading the bar pages against the new ones line-by-line, confirmed by every tool queried:
  1. **Inconsistent Card hover-lift.** `/safety-tips` and `/advisories` already used `transition-[box-shadow,transform] duration-200 ease-[var(--ease-feedback)] [@media(hover:hover)]:hover:-translate-y-0.5 [@media(hover:hover)]:hover:shadow-md` on their list Cards; `/cyber-awareness`, `/cyber-volunteers`, `/unlawful-content`, `/contact`, and the `/not-built/[category]` stub had plain, unlifted Cards. Fixed by applying the exact existing class string (no new token, no new value) to every list-style Card on those five surfaces. Single, non-list informational cards (the `/cyber-volunteers` disclaimer banner, the gold safety-note callouts) were deliberately left plain, matching the same precedent already set by `/safety-tips`'s own non-list CTA card.
  2. **Dropdown open/close using the wrong motion token, and too fast.** `components/ui/dropdown-menu.tsx`'s `DropdownMenuContent` ran Tailwind's generic `animate-in`/`zoom-in-95` at `duration-100` with no explicit easing — neither of the site's two established tokens (`--ease-standard`, the 380ms entrance curve; `--ease-feedback`, the fast UI-state curve already used for the Card hover-lift above). 100ms also sits below the 150–300ms micro-interaction range `ui-ux-pro-max`'s own checklist names. Fixed to `duration-150 ease-[var(--ease-feedback)]` — the feedback curve, not the entrance curve, since a menu open/close is a UI-state transition, not a one-time page reveal (this distinction, and the correction away from an initial plan to reuse the entrance token instead, came directly from `awwwards-ui-ux`'s rule 2, see honesty note below). `transition-colors` also added to `DropdownMenuItem` so the highlight state doesn't snap.
- **Tool-by-tool honesty (most queries validated the existing build; two contributed a real change):**
  - `ui-ux-pro-max` — validated the existing single-column dropdown (labels + separators, no grid mega-menu) as correctly right-sized for this content, and supplied the 150–300ms micro-interaction duration standard that flagged the dropdown's 100ms as too fast. Contributed the duration number, not the structure.
  - Magic MCP (`mcp__magic__search`) — searched dropdown/mega-menu and FAQ/glossary component patterns; returned the same plain Radix dropdown pattern already shipped (confirming no upgrade needed) and only marketing-style tabbed/searchable FAQ variants that don't fit a calm civic page (confirming the existing shadcn `Accordion` on `/faq` was correctly left alone). Pure validation, zero code changed from this query.
  - `impeccable` — its `init` flow requires a `PRODUCT.md` this hackathon prototype never had (`PROJECT_SPEC.md` already serves that role); rather than run a full product-setup flow mid-polish-pass, its critique rulebook (absolute-bans list, motion/interaction rules) was applied directly by hand against the six files in scope. Corroborated the two defects above already found by direct file comparison; surfaced nothing additional. `impeccable`'s setup script did leave a `PRODUCT.md` in the repo root as a side effect — left untracked and excluded from this commit, not deleted without confirmation.
  - `gpt-taste` — its entire toolkit (GSAP hero/hero hero architecture, gapless bento grids, marquees, `picsum.photos` stock imagery, AIDA page structure) is built for marketing/SaaS landing pages and was explicitly rejected wholesale for these read-heavy civic pages, per the task's own instruction to reject what doesn't fit. Nothing from this query was applied. Named here for honesty, not credited with a contribution.
  - `awwwards-ui-ux`, run last as the taste filter — confirmed the Card hover-lift extension was fine (already gated behind `@media (hover: hover)`, matching its rule 3), but **caught a real mistake before it shipped**: its rule 2 ("one signature easing curve for entrances, a separate fast curve for feedback — never the same curve for both") correctly argued against the initial plan to reuse `--ease-standard` (the entrance token) for the dropdown, since an open/close is feedback, not an entrance. The dropdown was built with `--ease-feedback` instead, per that correction. This is the one query in this phase that changed a decision already in flight, not just validated or supplied a number.
- **Untouched, as scoped:** homepage hero/cards, `/report/money`, `/track`, `lib/db/schema.ts`, `lib/types.ts`, OTP/session/rate-limit code, `components/chrome/site-footer.tsx`. `site-header.tsx`'s actual nav structure/copy from D54 is unchanged — only the shared `dropdown-menu.tsx` primitive's motion classes changed, which every dropdown in the header inherits.
- **Verification actually run:** `DATABASE_URL=postgres://cybercrime:cybercrime@localhost:5432/cybercrime npx tsc --noEmit` — 0 errors. `npm run lint` — 0 errors/warnings. `npm run build` — compiled successfully, all routes generated, no new errors (run twice: once with a temporary `<DropdownMenu open>` debug prop on the "Report & Check Suspect" trigger to force-render it for a screenshot, once after reverting — `git diff -- components/chrome/site-header.tsx` confirmed zero net change to that file). Screenshotted the open dropdown (labels/separators/external-link icons all rendering correctly, confirming the no-mega-menu-needed finding) and `/cyber-volunteers`, `/advisories`, `/cyber-awareness` at 1440×900, all viewed via the Read tool — calm, on-brand, no SaaS tells, hover-lift not visible in a static shot (expected, it's a `:hover` state) but confirmed present via source inspection. `npx @axe-core/cli` against all 5 new pages + the not-built stub + the homepage, in both `/en` and `/hi` (14 URLs total) — **0 violations on every URL**, both locales. Grepped all touched files for `indigo|purple|violet|bg-clip-text|gradient` — no hits (the pre-existing `brand-gold` washes on `/safety-tips`/`/unlawful-content` are untouched prior art, not new). Grepped for em dashes across `app/`, `components/`, `locales/` — all hits are pre-existing code comments (`//`), same convention as every prior phase; zero in user-facing copy or JSON.
- **Files changed this phase:** `components/ui/dropdown-menu.tsx`, `app/[locale]/{cyber-awareness,cyber-volunteers,unlawful-content,contact}/page.tsx`, `app/[locale]/not-built/[category]/page.tsx`, `PROJECT_SPEC.md` (this update, §31/§33). **Not modified in the final diff** (edited then reverted as part of the mandatory screenshot process, confirmed by empty `git diff`): `components/chrome/site-header.tsx`.
...
| **D55** | **A fourth UI/UX pass (`ui-ux-pro-max`/Magic MCP/`impeccable`/`gpt-taste`/`awwwards-ui-ux`) targeted only D54's nav dropdowns and five new content pages, and found two real, scoped defects rather than surfacing a new redesign** — inconsistent Card hover-lift (present on `/safety-tips`/`/advisories`, missing on `/cyber-awareness`/`/cyber-volunteers`/`/unlawful-content`/`/contact`/the `/not-built` stub) and the `DropdownMenuContent` open/close animation running Tailwind's generic `duration-100` default instead of either of the site's two existing motion tokens. Both fixed by reusing exactly what already existed (the established hover-lift class string; `--ease-feedback` at 150ms) — no new token, no new value, no new dependency | Following the D47 pattern of honesty about which tool contributed what: `ui-ux-pro-max` supplied the 150–300ms micro-interaction duration standard that flagged the dropdown's 100ms as too fast, and validated the existing single-column dropdown (no grid mega-menu needed). Magic MCP's dropdown/mega-menu and FAQ-pattern searches returned nothing better than what's already shipped — pure validation. `impeccable`'s `init` step demanded a `PRODUCT.md` this hackathon project never had; rather than run full product setup mid-polish-pass, its critique rulebook was applied by hand, which corroborated the two defects already found and surfaced nothing new (its setup script left an untracked `PRODUCT.md` in the repo root, excluded from this commit). `gpt-taste`'s entire GSAP/bento/marquee/stock-photo toolkit was explicitly rejected wholesale as wrong for a civic informational page, per the task's own instruction — zero code changed from this query. `awwwards-ui-ux`, run last as the taste filter, is the one query that changed a decision already in flight: its rule ("one signature easing curve for entrances, a separate fast curve for feedback — never the same curve for both") correctly overrode an initial plan to reuse the entrance token (`--ease-standard`) for the dropdown, redirecting it to the feedback token (`--ease-feedback`) instead, since an open/close is UI-state feedback, not a page-load entrance | §31 Phase I, `components/ui/dropdown-menu.tsx`, `app/[locale]/{cyber-awareness,cyber-volunteers,unlawful-content,contact}/page.tsx`, `app/[locale]/not-built/[category]/page.tsx` |
