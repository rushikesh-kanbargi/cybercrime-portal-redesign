...
export type SubmitHarassmentReportInput = z.infer<typeof submitHarassmentReportSchema>;

export interface SubmitHarassmentReportResult {
  publicId: string;
  complaintId: string;
  smsPreview: string;
}
...
export async function submitHarassmentReport(
  input: SubmitHarassmentReportInput,
): Promise<SubmitHarassmentReportResult> {
  const parsed = submitHarassmentReportSchema.parse(input);
  ...
  const complaintId = await db.transaction(async (tx) => {
    ...
    await tx.insert(complaintStatuses).values({
      complaintId: complaint.id,
      code: "RECEIVED",
      note: "Complaint received via the web harassment report flow.",
    });
    ...
    await tx.insert(auditLogs).values({
      ...
      metadata: { categoryCode: HARASSMENT_CATEGORY_CODE, subCategoryCode: parsed.subCategoryCode },
    });
