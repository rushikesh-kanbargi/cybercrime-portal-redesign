...
import {
  classifyAccountCompromise,
  ACCOUNT_COMPROMISE_CATEGORY_CODE,
  ACCOUNT_COMPROMISE_SUBCATEGORIES,
...
const DRAFT_KEY = "cc-hacked-draft-v1";

const PLATFORMS = ["instagram", "facebook", "whatsapp", "email", "banking", "gaming", "other"] as const;
...
function emptyDraft(): DraftState {
  const suggestion = classifyAccountCompromise("");
  return {
    narrative: "",
...
export function HackedReportWizard({ savedProfile }: { savedProfile?: SavedProfile | null }) {
  const t = useTranslations("reportHacked");
  ...
  const suggestion = React.useMemo(() => classifyAccountCompromise(draft.narrative), [draft.narrative]);
  ...
  function chooseCategory(code: AccountCompromiseSubCategoryCode) {
    setDraft((d) => ({
      ...d,
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const narrative = pastedEvidence
        ...
        : draft.narrative.trim();
      const res = await submitHackedReport({
        narrative,
  ...
  const categoryLabel = (code: AccountCompromiseSubCategoryCode) => t(`category.labels.${code}`);
          ...
          <CardContent className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-2">
              ...
              <FormSelect
                ...
                placeholder={t("facts.platformPlaceholder")}
                options={PLATFORMS.map((p) => ({ value: p, label: t(`facts.platforms.${p}`) }))}
              />
            </div>

            <div className="flex flex-col gap-2">
              <Label>{t("facts.accessLabel")}</Label>
              <div className="flex gap-2">
                <Button
                  type="button"
                  size="sm"
                  className="min-h-11"
                  variant={draft.stillHaveAccess ? "default" : "outline"}
                  onClick={() => setDraft((d) => ({ ...d, stillHaveAccess: true }))}
                >
                  {t("facts.accessYes")}
                </Button>
                <Button
                  ...
                  className="min-h-11"
                  variant={!draft.stillHaveAccess ? "default" : "outline"}
                  onClick={() => setDraft((d) => ({ ...d, stillHaveAccess: false }))}
                >
                  {t("facts.accessNo")}
                </Button>
              </div>
          ...
          <CardContent className="flex flex-col gap-4">
            ...
            <ReviewLine onEdit={() => setStep("facts")} editLabel={tCommon("actions.edit")}>
              {t("review.detailsLine", {
                ...
                access: draft.stillHaveAccess ? t("review.accessYes") : t("review.accessNo"),
              })}
            </ReviewLine>
            <ReviewLine onEdit={() => setStep("facts")} editLabel={tCommon("actions.edit")}>
...
function CategoryPicker({
  ...
  onChoose: (code: AccountCompromiseSubCategoryCode) => void;
  t: ReturnType<typeof useTranslations>;
  categoryLabel: (code: AccountCompromiseSubCategoryCode) => string;
}) {
  ...
  return (
    <div className="flex items-center gap-2">
      <FormSelect
        className="w-auto"
        value=""
        onValueChange={(v) => onChoose(v as AccountCompromiseSubCategoryCode)}
        placeholder={t("facts.choosePrompt")}
        options={ACCOUNT_COMPROMISE_SUBCATEGORIES.map((s) => ({ value: s.code, label: categoryLabel(s.code) }))}
      />
